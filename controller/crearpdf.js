...
var enviar= new CronJob('* * * * *', async function() {
  ...
  try{
      ...
      const user = ussers[index];
        ...
        doc.fontSize(15)
        doc.fillColor('blue')
        doc.text('Hola '+datospersonales[0].nombre+' '+datospersonales[0].apellido+' estas son las sugerencias de artículos para su compra:',80,80,{
          lineGap:40,
        }),
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          var foto = await modeloFotosVentas.findOne({where: {idVenta:record.idVenta}, order: [['indice', 'ASC']]})
          doc.image('./public/uploads/'+foto.dataValues.nombre,150,360,{fit:[300,200],align:'center',valign:'center'});
          doc.fillColor('black')
          doc.fontSize(18)
          doc.text('Para comprar este producto ingrese al siguiente enlace: ',100,580)
          doc.fontSize(17)
          .fillColor('blue')
